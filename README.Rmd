...
### Prelimanary: Step 2
```{r}
# Adjust the J200 data to include a Year_Month and rename the Sector
  Daily_J200_Adjusted <-
    J200 %>% 
    mutate(Year_Month = format(date, "%Y%B"))
  colnames(Daily_J200_Adjusted)[colnames(Daily_J200_Adjusted)=="ICB_INDUSTRY_NAME"] <- "Sector"
# Adjust the TRI data to include a Year_Month and rename the Sector and Industry columns
  Daily_TRI_Adjusted <- 
    Daily_TRI %>% 
    mutate(Year_Month = format(date, "%Y%B"))
...
## Assignment 1:
### Question 1.1.): Compare the Financials and Industrials returns for 2017
```{r}
# Step 1
...
```
### Question 1.2.) Find the 5 most volatile stocks for the index
```{r}
# Step 1 
...
# Step 2
  # Filter out the top 5 most volatile stocks for the index from 2017-2018
    SD_J200_Adjusted %>%
      top_n(5, SD)
```
### Question 1.3.) Use PerformanceAnalytics and compare the maximum drawdowns of the Industrials versus the Consumer Goods sectors
```{r}
# Step 1
...
## Assignment 2
### Question 2.1.) Calculate the cap-weighted monthly index returns for every sector for each country
```{r}
# Step 1
...
# Step 2
  # Calculate the Daily Cumulative Returns Column
    Daily_CumRet_TRI_Adjusted <- 
      ...
      ungroup()
# Step 3
  # Calculate the Monthly Returns column
    Sector_Monthly_Ret_TRI <-
      Daily_CumRet_TRI_Adjusted %>% 
      group_by(Year_Month, Ticker) %>%
      filter(date == last(date)) %>%
      arrange(date, Sector) %>% 
      group_by(Universe, Sector, date,Year_Month) %>% 
      summarise(Mean_Monthly_Returns = mean(Daily_Returns, na.rm = TRUE)) %>%
      ungroup() 
# Step 4
  # Create a graph that compares the sectors according to universe
    Sector_Monthly_Ret_TRI %>% 
      ggplot() + 
        geom_boxplot(aes(x = Sector, y = Mean_Monthly_Returns, fill = Sector)) +
        geom_jitter(aes(x = Sector, y = Mean_Monthly_Returns, color = Sector, alpha = 0.25)) +
...
```
### Question 2.2.) Calculate the sharpe ratio for the equally weighted by country, cap-weighted index returns since 2017
```{r}
# Step 1
  # Calculate Daily Returns column
    Daily_Ret_TRI_Adjusted <- 
      ...
      ungroup()
# Step 2
  # Calculate the Monthly Sharpe Ratios
    Monthly_Sharpe_TRI <- 
      Daily_Ret_TRI_Adjusted %>% 
      arrange(date, Year_Month) %>%
      group_by(Year_Month, Ticker) %>%
      filter(date == last(date)) %>%
      group_by(date, Year_Month, Universe) %>%
      summarise(Sharpe = mean(Daily_Returns, na.rm = TRUE) / sd(Daily_Returns, na.rm = TRUE)) %>%
      ungroup()
# Step 3
  # Create a graph that compares the Sharpe ratios per universe
    Monthly_Sharpe_TRI %>% 
      ggplot() + 
      ...
      guides(alpha = FALSE)
```
### Question 2.3.) Plot all the cumulative returns for the Financials indexes of the three countries
```{r}
# Step 1
 # Calculate Daily Returns column
    Daily_Ret_TRI_Adjusted <- 
...
# Step 3
  # Create a graph that compares the sectors according to universe
    ggplot(data = Financials_CumRet_TRI_Adjusted) + 
      ...
      theme_bw() + labs(title = "Financials Returns", y = "Returns")
```
### Question 2.4.) Calculate the JALSH index returns, by filtering the top 95% of Market Cap
```{r}
# Step 1
...
#Step 2
  # Calculate the JALSH index returns for the filtered (top 95% of Market Cap) JALSH index     
    Ret_MrktCap_JALSH <-
      MrktCap_JALSH %>%
      group_by(Ticker) %>% 
      mutate(Daily_Returns = TRI/lag(TRI) - 1) %>%
...
#Step 3
  # Plot the returns for the 95% JALSH index on a histogram
    Ret_MrktCap_JALSH %>% 
      ...
      theme_bw()
```
### Question 2.5.) Calculate and visually compare the rolling 60 day standard deviation for the Materials sectors of each country
```{r}
#Step 1
